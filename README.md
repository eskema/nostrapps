...
Opening a link shows one screen with the space's name (editable; one you already have must change), the apps, the actions it will run, and each new app's permissions, then opens them in an ephemeral space: nothing is installed, and a reload discards it (like `/dev`). Apps you already have run as your installed copy; the rest run as they would installed, only nothing about them persists. **Keep** makes them, and the space, stay — as they are. A link pasted into the launcher input, or just its `#…`, opens the same way.
