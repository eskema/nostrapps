...
export function mount(container, ctx, opts = {}) {
  ...
  const dTag = isEvent ? data.tags.find(t => t[0] === "d")?.[1] || "" : data.manifest?.dTag || ""
  const title = isEvent ? data.tags.find(t => t[0] === "title")?.[1] || "" : data.name || ""
  const description = isEvent ? data.tags.find(t => t[0] === "description")?.[1] || "" : ""
  const source = isEvent ? data.tags.find(t => t[0] === "source")?.[1] || "" : ""
  const paths = isEvent ? data.tags.filter(t => t[0] === "path").length : 0
  const createdAt = isEvent ? data.created_at : data.manifest?.createdAt || 0
  const handlers =
    data.handlers || (isEvent ? data.tags.filter(t => t[0] === "action").map(t => t[1]) : [])
    ...
    <div class="app-info-panel">
      ...
      ${
        ...
        <div class="app-info-handlers">
          ${handlers.map(h => `<span class="app-info-handler">${h}</span>`).join("")}
        </div>
      `
          : ""
      }
